...
// File loading
// ---------------------------------------------------------------------------
// baseDir: if non-empty, f.name is set to the path relative to baseDir
// (so files in subfolders are distinguishable). Otherwise just the filename.
static void addFile(const std::wstring& path, const std::wstring& baseDir = L"") {
    for (auto& f : g_files) if (f.path == path) return; // dedupe by full path
    DzeFile f;
    f.path = path;
    if (!baseDir.empty() && path.size() > baseDir.size() &&
        _wcsnicmp(path.c_str(), baseDir.c_str(), baseDir.size()) == 0) {
        size_t start = baseDir.size();
        while (start < path.size() && (path[start] == L'\\' || path[start] == L'/')) start++;
        f.name = path.substr(start);            // relative path incl. subfolders
    } else {
...
// Recursive scan: adds every .dze under `dir` (including subfolders).
// `baseDir` stays fixed to the originally-selected folder so relative
// display names include the subfolder path. `depth` guards runaway nesting.
static void scanFolder(const std::wstring& dir, const std::wstring& baseDir, int depth = 0) {
    if (depth > 32) return; // safety against symlink loops / absurd nesting
    ...
    do {
        ...
        if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            scanFolder(full, baseDir, depth + 1);
        } else {
            size_t dot = nm.find_last_of(L'.');
            if (dot != std::wstring::npos) {
                std::wstring ext = nm.substr(dot);
                if (_wcsicmp(ext.c_str(), L".dze") == 0)
                    addFile(full, baseDir);
            }
        }
...
static void doAddFolder() {
    ...
    if (SHGetPathFromIDListW(pidl, path)) scanFolder(path, path);
    CoTaskMemFree(pidl);
    runAnalysis();
}
